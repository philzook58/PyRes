...
def generateEquivAxioms():
    ...
    while not lex.TestTok(Token.EOFToken):
        c = parseClause(lex)
        c.setDerivation(Derivation("theory(equality)"))
        res.append(c)
    return res
...
def generateEqPremise(arity):
    ...
    """
    res = [
        Literal(list(["=", vars[0], vars[1]]), True)
        for vars in zip(generateVarList("X", arity), generateVarList("Y", arity))
    ]
    return res
...
def generateFunCompatAx(f, arity):
    ...
    resclause = Clause(res, "axiom")
    resclause.setDerivation(Derivation("theory(equality)"))
    return resclause
